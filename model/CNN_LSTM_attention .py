...
def main(save_confusion=False):
    ...
    X_train_cont1, X_test_cont1, y_train1, y_test1 = train_test_split(x_cont, y, test_size=0.01, random_state=25,stratify=y)
    X_train_eye1, X_test_eye1, y_train1, y_test1 = train_test_split(x_eye, y, test_size=0.01, random_state=25,stratify=y)
    X_train_phy1, X_test_phy1, y_train1, y_test1 = train_test_split(x_phy, y, test_size=0.01, random_state=25,stratify=y)
    ...
    for k, (train_index, test_index) in enumerate(kf.split(X_train_eye1)):
        if k >1:
            ...
            history = model.fit(x_train, y_train, batch_size=16, epochs=100, verbose=1, validation_split=0.1, shuffle=True,
                                               callbacks=[es])
